...
async function listDirsOnceAsync(base: string): Promise<string[]> {
  ...
  for (const name of names) {
    ...
    try {
      if ((await fs.stat(p)).isDirectory()) out.push(p);
    } catch {
      /* empty */
    }
  }
...
async function parseWebBaseFromCommonPath(base: string): Promise<string> {
  ...
  if (!text) {
    log("[WARN] common_path.js 읽기 실패 → fallback=http://127.0.0.1");
...
async function isPublicReachable(signal?: AbortSignal): Promise<boolean> {
  ...
  try {
    ...
    if (!res.ok) {
      log(`[PUBLIC] 응답 ${res.status} → 비공개로 간주`);
      return false;
    }
    return true;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  async #run() {
    while (!this.#stopCtrl.signal.aborted) {
      try {
        ...
        if (!pub) {
          await delay(15_000, undefined, { signal: this.#stopCtrl.signal }).catch(() => {
            /* empty */
          });
          continue;
        }
        ...
        if (hidden) {
          ...
          if (waitMs > 0) {
            // 신호 안전하게 대기
            await delay(waitMs, undefined, { signal: this.#stopCtrl.signal }).catch(() => {
              /* empty */
            });
            if (this.#stopCtrl.signal.aborted) break;
          }
          log(
            `[${this.name()}] 닫힘 - (정렬) autoreset 호출 @ mm:00${AUTO_RESET_JITTER_MS ? `+${AUTO_RESET_JITTER_MS}ms` : ""}`,
          );
          await httpGetJson<Json>(this.#entry.autoresetUrl, this.#stopCtrl.signal);
          // 같은 분에 중복 호출 방지: 0초 직후 잠깐 쉬고 다음 루프는 자연스럽게 다음 분으로 정렬됨
          await delay(200, undefined, { signal: this.#stopCtrl.signal }).catch(() => {
            /* empty */
          });
          continue;
        }
        ...
        if (!data) {
          await delay(IF_LOCKED_WAIT_MS, undefined, { signal: this.#stopCtrl.signal }).catch(() => {
            /* empty */
          });
          continue;
        }
        ...
        if (data.updated === true) {
          ...
          await delay(IF_UPDATED_WAIT_MS, undefined, { signal: this.#stopCtrl.signal }).catch(() => {
            /* empty */
          });
          continue;
        }

        await delay(DEFAULT_WAIT_MS, undefined, { signal: this.#stopCtrl.signal }).catch(() => {
          /* empty */
        });
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
      } catch (e: any) {
        ...
        await delay(5_000, undefined, { signal: this.#stopCtrl.signal }).catch(() => {
          /* empty */
        });
      }
    }
  ...
  #scheduleRescan() {
    ...
    const loop = async () => {
      while (!this.#rescanCtrl.signal.aborted) {
        await delay(RESCAN_INTERVAL_MS, undefined, { signal: this.#rescanCtrl.signal }).catch(() => {
          /* empty */
        });
        ...
        try {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          log("[RESCAN] 예외:", e?.message || String(e));
        }
      }
    };
    loop().catch(() => {
