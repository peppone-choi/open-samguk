...
async function changeInstallMode() {
  ...
  if (result.step == "sql_fail") {
    ...
    $("#admin_form_card").hide();
    alert(
      "DB가 제대로 설정되지 않았거나, 훼손된 것 같습니다. DB를 복구하거나 RootDB.php 파일을 삭제 후 재설치를 진행해 주십시오.",
    );
    return;
  }
...
function setupDBForm() {
  ...
  $("#serv_host").val([location.protocol, "//", location.host, parentPathname].join(""));
...
function setupAdminForm() {
  ...
  const descriptor: NamedRules<AdminValues> = {
    username: {
      ...
      type: "string",
      options: {
        //FIXME: options.messages가 동작하지 않는다?
        messages: {
