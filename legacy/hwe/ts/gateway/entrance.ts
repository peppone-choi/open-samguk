...
async function Entrance_drawServerList(serverInfos: ServerResponseItem[]) {
  ...
  for (const serverInfo of serverInfos) {
    ...
    if (response.reserved) {
      ...
      $serverHtml.append(TemplateEngine(serverReservedTemplate, response.reserved));
    ...
    if (game.opentime <= now) {
      $serverHtml.append(TemplateEngine(serverTextInfo, game));
    } else {
      $serverHtml.append(TemplateEngine(serverProvisionalInfo, game));
    }

    if (response.me && response.me.name) {
      const me = response.me;
      me.serverPath = serverPath;
      $serverHtml.append(TemplateEngine(serverLoginTemplate, me));
    } else if (game.userCnt >= game.maxUserCnt) {
      $serverHtml.append(TemplateEngine(serverFullTemplate, {}));
    } else {
      $serverHtml.append(
