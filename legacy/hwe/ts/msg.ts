...
function redrawMsg(msgResponse: MsgResponse, addFront: boolean): MsgResponse {
  ...
  function printTemplate(obj: MsgResponse) {
    ...
    for (const [msgSource, $msgBoard, msgType] of printList) {
      ...
      const $msgs: JQuery<HTMLElement>[] = msgSource
        .map(function (msg) {
          const contactTarget =
            msg.src.id != myGeneralID || msg.msgType === "public" ? msg.src.id : unwrap(msg.dest).id;
...
function refreshMailboxList(obj: MabilboxListResponse) {
  ...
  //아국메시지, 전체메시지
  const $ourCountry = $(`<option value="${unwrap(myNation).mailbox}">【 아국 메세지 】</option>`).css({
    "background-color": unwrap(myNation).color,
    color: isBrightColor(unwrap(myNation).color) ? "black" : "white",
  });
