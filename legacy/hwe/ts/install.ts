...
async function loadScenarios() {
  ...
  for (const [category, items] of Object.entries(list)) {
    ...
    for (const [idx, scenario] of Object.entries(items)) {
      const $option = $("<option>").data("scenario", scenario).val(idx).html(scenario.title);
...
function scenarioPreview() {
  ...
  $.each(scenario.nation, function (idx, nation) {
    $nation.append(
      `<span style="color:${nation.color}">${nation.name}</span> ${nation.generals}명. ${nation.cities.join(", ")}<br>`,
    );
  });
}
