...
export class StoredActionsHelper {
  ...
  constructor(
    protected serverNick: string,
    protected type: "general" | "nation",
    protected mapName: string,
    protected unitSet: string,
    protected maxRecent = 10,
  ) {
  ...
  saveStoredActions() {
    localStorage.setItem(this.storedActionsKey, JSON.stringify(Array.from(this.storedActions.value.entries())));
  }
}
