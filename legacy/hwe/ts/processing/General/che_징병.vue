<template>
  ...
  <div class="bg0">
    <div>
      병사를 모집합니다.
      <template v-if="commandName == '징병'"> 훈련과 사기치는 낮지만 가격이 저렴합니다.<br /> </template>
      <template v-else-if="commandName == '모병'"> 훈련과 사기치는 높지만 자금이 많이 듭니다.<br /> </template>
      가능한 수보다 많게 입력하면 가능한 최대 병사를 모집합니다.<br />
      이미 병사가 있는 경우 추가 {{ commandName }}되며, 병종이 다를경우는 기존의 병사는 소집해제됩니다. <br />
      현재 {{ commandName }} 가능한 병종은 <span style="color: green">녹색</span>으로 표시되며, 현재
      {{ commandName }} 가능한 특수병종은 <span style="color: limegreen">초록색</span>으로 표시됩니다.
    </div>
    <div ref="defaultTarget" class="crewTypeList">
      <div class="listFront">
        <div class="row gx-0 bg0">
          <div class="col-12 col-lg-12 d-flex align-items-center">
            <div v-if="commandName == '모병'" class="text-center w-100">모병은 가격 2배의 자금이 소요됩니다.<br /></div>
          </div>
        </div>
        <div class="row text-center bg2 gx-0">
          <div class="col-4 col-lg-2">현재 기술력 : {{ techLevel }}등급</div>
          ...
          <div class="col-4 col-lg-2">최대 통솔 : {{ fullLeadership }}</div>
          <div class="col-4 col-lg-2">현재 병종 : {{ crewTypeMap?.get(currentCrewType)?.name }}</div>
          <div class="col-4 col-lg-2">현재 병사 : {{ crew.toLocaleString() }}</div>
          <div class="col-4 col-lg-2">현재 자금 : {{ gold.toLocaleString() }}</div>
        </div>
        <div class="miniCrewPanel center bg0">
          ...
          <div
            :style="{
              backgroundColor:
                (destCrewType.notAvailable ? 'red' : destCrewType.reqTech == 0 ? 'green' : 'limegreen') + ' !important',
              height: '100%',
            }"
            class="d-grid"
          >
                ...
                <div class="input-group my-0">
                  ...
                  <span
                    ...
                    ><div style="margin-left: auto">
                      {{ Math.ceil(amount * destCrewType.baseCost * goldCoeff).toLocaleString() }}금
                    </div></span
                  >
                </div>
              </div>
      ...
      <div class="listMain">
        <template v-for="armCrewType in armCrewTypes" :key="armCrewType.armType">
          <div class="s-border-b row gx-0">
            <div class="col-7 col-lg-10 align-self-center px-3" style="font-size: 1.3em">
          ...
          <template v-for="crewType in armCrewType.values" :key="crewType.id">
            <CrewTypeItem
              v-if="showNotAvailable.get(armCrewType.armType) || !crewType.notAvailable"
              :crewType="crewType"
...
const { techLevel, goldCoeff, leadership, fullLeadership, armCrewTypes, currentCrewType, crew, gold } = procRes;
...
function beFilled() {
  if (destCrewType.value.id == procRes.currentCrewType) {
    amount.value = Math.max(1, procRes.fullLeadership - Math.floor(procRes.crew / 100));
  } else {
