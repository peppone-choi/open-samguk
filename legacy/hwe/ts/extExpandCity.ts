  ...
  const loadUser = function () {
    $.each(cityList, function (idx, val) {
      if (typeof val.users == "undefined") {
        val.obj.append(
          '<tr><td colspan="10"><table align="center" class="tb_layout cityUser bg0">' +
            "<thead><tr>" +
    ...
    void $.get(basicPath + "b_genList.php", function (rawData) {
      ...
      tmpUsers.each(function () {
        ...
        if (cityInfo.warn주민)
          $work.html($work.html().split("정착 장려").join('<span style="color:yellow;">정착 장려</span>'));
        if (cityInfo.warn농업)
          $work.html($work.html().split("농지 개간").join('<span style="color:yellow;">농지 개간</span>'));
        if (cityInfo.warn상업)
          $work.html($work.html().split("상업 투자").join('<span style="color:yellow;">상업 투자</span>'));
        if (cityInfo.warn치안)
          $work.html($work.html().split("치안 강화").join('<span style="color:yellow;">치안 강화</span>'));
        if (cityInfo.warn수비)
          $work.html($work.html().split("수비 강화").join('<span style="color:yellow;">수비 강화</span>'));
        if (cityInfo.warn성벽)
          $work.html($work.html().split("성벽 보수").join('<span style="color:yellow;">성벽 보수</span>'));
  ...
  const mainFunc = function () {
    ...
    $("table").each(function () {
...

      {
        ...
        if (cityInfo.농업 > cityInfo.max농업 * 0.8) {
          cityInfo.$농업.css("color", "lightgreen");
        } else if (cityInfo.농업 > cityInfo.max농업 * 0.4) {
          cityInfo.$농업.css("color", "yellow");
        } else {
          cityInfo.$농업.css("color", "orangered");
        }

        if (cityInfo.상업 > cityInfo.max상업 * 0.8) {
          cityInfo.$상업.css("color", "lightgreen");
        } else if (cityInfo.상업 > cityInfo.max상업 * 0.4) {
          cityInfo.$상업.css("color", "yellow");
        } else {
          cityInfo.$상업.css("color", "orangered");
        }

        if (cityInfo.치안 > cityInfo.max치안 * 0.8) {
          cityInfo.$치안.css("color", "lightgreen");
        } else if (cityInfo.치안 > cityInfo.max치안 * 0.4) {
          cityInfo.$치안.css("color", "yellow");
        } else {
          cityInfo.$치안.css("color", "orangered");
        }

        if (cityInfo.수비 > cityInfo.max수비 * 0.6) {
          cityInfo.$수비.css("color", "lightgreen");
        } else if (cityInfo.수비 > cityInfo.max수비 * 0.3) {
          cityInfo.$수비.css("color", "yellow");
        } else {
          cityInfo.$수비.css("color", "orangered");
        }

        if (cityInfo.성벽 > cityInfo.max성벽 * 0.6) {
          cityInfo.$성벽.css("color", "lightgreen");
        } else if (cityInfo.성벽 > cityInfo.max성벽 * 0.3) {
          cityInfo.$성벽.css("color", "yellow");
        } else {
          cityInfo.$성벽.css("color", "orangered");
        }
        ...
        if (cityInfo.warn농업)
          cityInfo.$농업.append('<span class="remain" style="color:yellow;">[' + cityInfo.remain농업 + "]</span>");
        if (cityInfo.warn상업)
          cityInfo.$상업.append('<span class="remain" style="color:yellow;">[' + cityInfo.remain상업 + "]</span>");
        if (cityInfo.warn치안)
          cityInfo.$치안.append('<span class="remain" style="color:yellow;">[' + cityInfo.remain치안 + "]</span>");
        if (cityInfo.warn수비)
          cityInfo.$수비.append('<span class="remain" style="color:yellow;">[' + cityInfo.remain수비 + "]</span>");
        if (cityInfo.warn성벽)
          cityInfo.$성벽.append('<span class="remain" style="color:yellow;">[' + cityInfo.remain성벽 + "]</span>");
      }
