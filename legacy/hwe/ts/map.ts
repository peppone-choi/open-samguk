...
export async function reloadWorldMap(option: loadMapOption, drawTarget = ".world_map"): Promise<void> {
  ...
  async function checkReturnObject(obj: MapRawResult): Promise<MapResult> {
    ...
    if (!isNumber(obj.startYear) || !isNumber(obj.year) || !isNumber(obj.month)) {
  ...
  function drawDetailWorldMap(obj: MapCityDrawable) {
    ...
    cityList.forEach(function (city) {
      ...
      const $imgObj = $(
        `<div class="city_img"><img src="${window.pathConfig.gameImage}/cast_${city.level}.gif"><div class="city_filler"></div></div>`,
      );
      $linkObj.append($imgObj);

      if (city.state > 0) {
        const $stateObj = $(
          `<div class="city_state"><img src="${window.pathConfig.gameImage}/event${city.state}.gif"></div>`,
        );
      ...
      if (city.nationID && city.nationID > 0) {
        ...
        const $flagObj = $(
          `<div class="city_flag"><img src="${window.pathConfig.gameImage}/${flagType}${convColorValue(unwrap(city.color))}.gif"></div>`,
        );
        if (city.isCapital) {
          const $capitalObj = $(
            `<div class="city_capital"><img src="${window.pathConfig.gameImage}/event51.gif"></div>`,
          );
