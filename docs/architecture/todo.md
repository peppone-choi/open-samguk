...
## Rewrite Execution Checklist (Tick-off)

- [ ] Follow `docs/architecture/implementation-checklist.md` and tick items as work progresses.
- [ ] Phase A: Workspace scripts + `packages/common` build/test stable
- [ ] Phase B: `packages/infra` (DB/Redis/env) skeleton
- [ ] Phase C: `packages/logic` pure domain skeleton + deterministic unit test
- [ ] Phase D: `apps/engine` turn daemon MVP (status/run/pause/resume)
- [ ] Phase E: `apps/api` NestJS + tRPC MVP (daemon control endpoints)
- [ ] Phase F: `apps/web` Next.js MVP (status + buttons)
- [ ] Phase G: Redis Streams control channel (requestId/idempotency)
- [ ] Phase H: snapshot/journal persistence skeleton
- [ ] Phase I: legacy parity test harness (stable inputs/outputs)
- [ ] Phase J: commands/constraints/triggers MVP (1~3 commands first)
- [ ] Phase K: observability/ops minimum (health, correlated logs)
