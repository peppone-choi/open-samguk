...
- **현황**: **완료 (100%)** ✅
- **작업 내용**:
  - `basic.json`, `che_except_siege.json`, `cr.json` 등 모든 레거시 병종 데이터를 JSON으로 이식 완료.
  - 약 20+종의 특수 병종(청주병, 호표기 등) 데이터 및 상성 보정 로직 구현 완료.
  - `WarEngine`에서 병종 어빌리티가 실제 전투 페이즈에 반영되도록 통합 완료.
