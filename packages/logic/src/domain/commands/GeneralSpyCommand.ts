...
export class GeneralSpyCommand extends GeneralCommand {
    ...
    this.fullConditionConstraints = [
      ...
      // 아국이 아닌 도시여야 함 (NotOccupiedDestCity)
      {
        ...
        test: (ctx, view) => {
          ...
          const destCity = view.get({ kind: 'destCity', id: ctx.args.destCityId });
  ...
  run(rng: RandUtil, snapshot: WorldSnapshot, actorId: number, args: Record<string, any>): WorldDelta {
    ...
    if (check.kind === 'deny') {
      return { logs: { general: { [actorId]: [`첩보 실패: ${check.reason}`] } } };
    }

    const iGeneral = snapshot.generals[actorId];
    if (!iGeneral) throw new Error(`장수 ${actorId}를 찾을 수 없습니다.`);
    if (iGeneral.gold < reqGold || iGeneral.rice < reqRice) {
      return { logs: { general: { [actorId]: [`첩보 실패: 자금이나 군량이 부족합니다.`] } } };
    }

    const destCityId = args.destCityId;
    const destCity = snapshot.cities[destCityId];
    if (!destCity) return { logs: { general: { [actorId]: [`첩보 실패: 대상 도시를 찾을 수 없습니다.`] } } };

    const dist = MapUtil.getDistance(iGeneral.cityId, destCityId);
    
    // 대상 도시 장수 및 병력 정보 수집
    const cityGenerals = Object.values(snapshot.generals).filter(g => g.cityId === destCityId && g.nationId === destCity.nationId);
    ...
    if (dist <= 1) {
      ...
      generalLogs.push(`【첩보】농업:${agriText}, 상업:${commText}, 치안:${secuText}, 수비:${defText}, 성벽:${wallText}`);
      ...
      const crewTypeText = Object.entries(crewTypes).map(([type, count]) => `병종${type}:${count}`).join(' ');
