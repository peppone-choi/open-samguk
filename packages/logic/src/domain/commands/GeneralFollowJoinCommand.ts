...
export class GeneralFollowJoinCommand extends GeneralCommand {
  ...
  run(rng: RandUtil, snapshot: WorldSnapshot, actorId: number, args: Record<string, any>): WorldDelta {
    ...
    if (!destGeneralId) {
      return {
        logs: {
          general: { [actorId]: ['장수대상임관 실패: 대상 장수가 지정되지 않았습니다.'] },
        },
      };
    }
    ...
    if (!destGeneral) {
      return {
        logs: {
          general: { [actorId]: ['장수대상임관 실패: 존재하지 않는 장수입니다.'] },
        },
      };
    }
    ...
    if (destGeneral.nationId === 0) {
      return {
        logs: {
          general: { [actorId]: ['장수대상임관 실패: 대상 장수가 재야입니다.'] },
        },
      };
    }
    ...
    if (!destNation) {
      return {
        logs: {
          general: { [actorId]: ['장수대상임관 실패: 국가가 존재하지 않습니다.'] },
        },
      };
    }
    ...
    return {
      ...
      logs: {
        ...
        global: [`${iGeneral.name} 장수가 ${destGeneral.name}을(를) 따라 ${destNation.name}에 임관했습니다.`],
      },
    };
  }
}
