...
import { NationProposeNonAggressionCommand } from "./NationProposeNonAggressionCommand.js";
...
describe("NationProposeNonAggressionCommand", () => {
  ...
  it("평화 상태에서 불가침 제의 시 성공해야 함 (6개월 이상 기한)", () => {
    const cmd = new NationProposeNonAggressionCommand();
    ...
    expect(delta.messages?.[0]?.meta?.action).toBe("propose_non_aggression");
    // 로그 확인
    expect(delta.logs?.general?.[1][0]).toContain("불가침 제의 서신");
  });

  it("6개월 미만 기한으로는 불가침 제의할 수 없어야 함", () => {
    const cmd = new NationProposeNonAggressionCommand();
    const delta = cmd.run(rand, baseSnapshot, 1, {
      destNationId: 2,
      year: 186,
