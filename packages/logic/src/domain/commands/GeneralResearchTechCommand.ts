...
export class GeneralResearchTechCommand extends GeneralCommand {
  ...
  run(rng: RandUtil, snapshot: WorldSnapshot, actorId: number, args: Record<string, any>): WorldDelta {
    ...
    if (iGeneral.gold < reqGold) {
        return {
            logs: {
                general: { [actorId]: [`기술 연구 실패: 자금이 부족합니다. (필요: ${reqGold})`] },
            },
        };
    }
    ...
    // 기술 연구 로직 수행
    const { delta: generalDelta, techGain, pick } = general.researchTech(
        rng, 
        iGeneral.intel, 
        iCity.trust, 
    ...
    const gennum = Object.values(snapshot.generals).filter(g => g.nationId === iGeneral.nationId).length || 1;
