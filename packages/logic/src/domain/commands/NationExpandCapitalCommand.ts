  ...
  run(
    ...
    if (!iNation)
      return {
        logs: {
          general: {
            [actorId]: ["증축 실패: 소속 국가 정보를 찾을 수 없습니다."],
          },
        },
      };
    ...
    if (!iNation.capitalCityId || iNation.capitalCityId === 0) {
      return {
        logs: {
          general: { [actorId]: ["증축 실패: 방랑 상태에서는 불가능합니다."] },
        },
      };
    }
    ...
    if (!iCapital) {
      return {
        logs: {
          general: { [actorId]: ["증축 실패: 수도 정보를 찾을 수 없습니다."] },
        },
      };
    }
    ...
    if (iCapital.level <= 3) {
      return {
        logs: {
          general: {
            [actorId]: ["증축 실패: 수진, 진, 관문에서는 불가능합니다."],
          },
        },
      };
    }
    ...
    if (iCapital.level >= 8) {
      return {
        logs: {
          general: { [actorId]: ["증축 실패: 더이상 증축할 수 없습니다."] },
        },
      };
    }
