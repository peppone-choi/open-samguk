...
export class NationAcceptPeaceCommand extends GeneralCommand {
  ...
  run(rng: RandUtil, snapshot: WorldSnapshot, actorId: number, args: Record<string, any>): WorldDelta {
    ...
    if (!iDiplomacy || (iDiplomacy.state !== '0' && iDiplomacy.state !== '1')) {
      return { logs: { general: { [actorId]: ['종전 수락 실패: 상대국과 선포, 전쟁중이지 않습니다.'] } } };
    }
    ...
    return {
      ...
      logs: {
        general: {
          ...
          [destGeneralId]: [`【${iNation.name}】${JosaUtil.pick(iNation.name, '와')} 종전에 성공했습니다.`],
        },
        nation: {
