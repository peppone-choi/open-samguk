...
export class NationAidCommand extends GeneralCommand {
  ...
  run(
    ...
    if (
      ...
      !amountList ||
      !Array.isArray(amountList)
    ) {
      return {
        logs: {
          general: {
            [actorId]: [
              "원조 실패: 대상 국가 또는 원조량이 지정되지 않았습니다.",
            ],
          },
        },
      };
    ...
    if (typeof goldAmount !== "number" || typeof riceAmount !== "number") {
      return {
        logs: { general: { [actorId]: ["원조 실패: 잘못된 원조량입니다."] } },
      };
    }
    ...
    if (goldAmount < 0 || riceAmount < 0) {
      return {
        logs: {
          general: { [actorId]: ["원조 실패: 음수는 원조할 수 없습니다."] },
        },
      };
    }
    ...
    if (goldAmount === 0 && riceAmount === 0) {
      return {
        logs: { general: { [actorId]: ["원조 실패: 원조량이 0입니다."] } },
      };
    }
    ...
    if (iDestNation.id === iNation.id) {
      return {
        logs: {
          general: { [actorId]: ["원조 실패: 자국에게는 원조할 수 없습니다."] },
        },
      };
    }
    ...
    if (surlimit > 0) {
      return {
        logs: { general: { [actorId]: ["원조 실패: 외교제한중입니다."] } },
      };
    }
    ...
    if (destSurlimit > 0) {
      return {
        logs: {
          general: { [actorId]: ["원조 실패: 상대국이 외교제한중입니다."] },
        },
      };
    }
        ...
        [destNationId]: {
          ...
          meta: {
            ...
            recv_assist: {
              ...
              [`n${iNation.id}`]: [
                iNation.id,
                (iDestNation.meta?.recv_assist?.[`n${iNation.id}`]?.[1] || 0) +
                  actualGold +
                  actualRice,
              ],
            },
