...
export class NationChangeCapitalCommand extends GeneralCommand {
  ...
  private getDistance(snapshot: WorldSnapshot, nationId: number, destCityId: number): number | undefined {
  ...
  run(rng: RandUtil, snapshot: WorldSnapshot, actorId: number, args: Record<string, any>): WorldDelta {
    ...
    if (!iNation) return { logs: { general: { [actorId]: ['천도 실패: 소속 국가 정보를 찾을 수 없습니다.'] } } };

    const { destCityId } = args;
    if (destCityId === undefined) return { logs: { general: { [actorId]: ['천도 실패: 목적지 도시가 지정되지 않았습니다.'] } } };

    const iDestCity = snapshot.cities[destCityId];
    if (!iDestCity || iDestCity.nationId !== iActor.nationId) {
        return { logs: { general: { [actorId]: ['천도 실패: 자국 영토가 아닌 곳으로는 천도할 수 없습니다.'] } } };
    }

    if (iNation.capitalCityId === destCityId) {
        return { logs: { general: { [actorId]: ['천도 실패: 이미 수도인 곳입니다.'] } } };
    }

    const distance = this.getDistance(snapshot, iActor.nationId, destCityId);
    if (distance === undefined) {
        return { logs: { general: { [actorId]: ['천도 실패: 목적지 도시까지의 경로가 차단되었습니다.'] } } };
    }
    ...
    if (iNation.gold < cost || iNation.rice < cost) {
        return { logs: { general: { [actorId]: [`천도 실패: 자원(금/쌀)이 부족합니다. (필요: ${cost})`] } } };
    }

    const preReqTurn = distance * 2;
    const lastTurn = iActor.lastTurn || {};
    const currentTerm = (lastTurn.action === this.actionName && lastTurn.destCityId === destCityId) ? (lastTurn.term || 0) : 0;
    ...
    return {
      ...
      logs: {
        ...
        nation: {
          [iNation.id]: [`${iActor.name}${josaYi} 【${iDestCity.name}】${josaRo} 천도 명령을 완료하였습니다.`],
        },
        global: [
