  ...
  run(
    ...
    if (!iNation)
      return {
        logs: {
          general: {
            [actorId]: ["발령 실패: 소속 국가 정보를 찾을 수 없습니다."],
          },
        },
      };
    ...
    if (destGeneralId === actorId) {
      return {
        logs: {
          general: { [actorId]: ["발령 실패: 본인을 발령할 수 없습니다."] },
        },
      };
    }
    ...
    if (!iDestGeneral) {
      return {
        logs: {
          general: { [actorId]: ["발령 실패: 대상 장수를 찾을 수 없습니다."] },
        },
      };
    }
    ...
    if (iDestGeneral.nationId !== iActor.nationId) {
      return {
        logs: {
          general: {
            [actorId]: ["발령 실패: 타국 장수는 발령할 수 없습니다."],
          },
        },
      };
    }
    ...
    if (!iDestCity) {
      return {
        logs: {
          general: { [actorId]: ["발령 실패: 대상 도시를 찾을 수 없습니다."] },
        },
      };
    }
    ...
    if (iDestCity.nationId !== iActor.nationId) {
      return {
        logs: {
          general: {
            [actorId]: [
              "발령 실패: 자국 영토가 아닌 곳으로는 발령할 수 없습니다.",
            ],
          },
        },
      };
    ...
    return {
      ...
      logs: {
        general: {
          ...
          [destGeneralId]: [
            `【${iActor.name}】에 의해 【${iDestCity.name}】${josaRo} 발령됐습니다.`,
          ],
        },
      },
    };
