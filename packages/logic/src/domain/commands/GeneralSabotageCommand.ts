...
export abstract class GeneralSabotageCommand extends GeneralCommand {
  ...
  run(
    ...
    if (check.kind === "deny") {
      return {
        logs: { general: { [actorId]: [`실행 불가: ${check.reason}`] } },
      };
    }
    ...
    const prob =
      (GameConst.sabotageDefaultProb +
        this.calcAttackProb(rng, snapshot, actorId) -
        this.calcDefenceProb(rng, snapshot, destCityId, destCity.nationId)) /
      dist;
