...
export class GeneralEquipmentTradeCommand extends GeneralCommand {
  ...
  run(rng: RandUtil, snapshot: WorldSnapshot, actorId: number, args: Record<string, any>): WorldDelta {
    ...
    if (!['weapon', 'horse', 'book', 'item'].includes(itemType)) {
        return { logs: { general: { [actorId]: ['장비 타입이 올바르지 않습니다.'] } } };
    }
    ...
    if (isSelling) {
        ...
        if (targetItemCode === 'None' || !targetItemCode) {
            return { logs: { general: { [actorId]: ['판매할 장비가 없습니다.'] } } };
        }
    }
    ...
    if (!itemInfo) {
        return { logs: { general: { [actorId]: ['해당 장비 정보를 찾을 수 없습니다.'] } } };
    }
    ...
    if (!isSelling) {
        ...
        if ((iGeneral as any)[itemType] === targetItemCode) {
            this.fullConditionConstraints.push(ConstraintHelper.AlwaysFail('이미 소지하고 있는 장비입니다.'));
        }
    }
