 ...
 * - 기한은 6개월 이상이어야 함
 *
 * Note: 레거시와 달리 OccupiedCity/SuppliedCity 제약 없음
 */
  ...
  run(
    ...
    if (!iNation)
      return {
        logs: {
          general: {
            [actorId]: ["불가침 제의 실패: 소속 국가 정보를 찾을 수 없습니다."],
          },
        },
      };
    ...
    if (
      ...
      return {
        logs: {
          general: {
            [actorId]: [
              "불가침 제의 실패: 대상 국가 또는 기한이 지정되지 않았습니다.",
            ],
          },
        },
      };
    ...
    if (!iDestNation) {
      return {
        logs: {
          general: {
            [actorId]: ["불가침 제의 실패: 대상 국가를 찾을 수 없습니다."],
          },
        },
      };
    }
    ...
    if (iDestNation.id === iNation.id) {
      return {
        logs: {
          general: {
            [actorId]: [
              "불가침 제의 실패: 자국에게는 불가침을 제의할 수 없습니다.",
            ],
          },
        },
      };
    ...
    if (reqMonth < currentMonth + 6) {
      return {
        logs: {
          general: {
            [actorId]: ["불가침 제의 실패: 기한은 6개월 이상이어야 합니다."],
          },
        },
      };
    }
    ...
    if (iDiplomacy) {
      if (iDiplomacy.state === "0") {
        return {
          logs: {
            general: {
              [actorId]: ["불가침 제의 실패: 아국과 이미 교전중입니다."],
            },
          },
        };
      }
      if (iDiplomacy.state === "1") {
        return {
          logs: {
            general: {
              [actorId]: ["불가침 제의 실패: 아국과 이미 선포중입니다."],
            },
          },
        };
      }
    ...
    return {
      ...
      logs: {
        general: {
          [actorId]: [
            `【${iDestNation.name}】${josaRo} 불가침 제의 서신을 보냈습니다.`,
          ],
        },
      },
    };
