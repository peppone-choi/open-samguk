...
describe("NationProposePeaceCommand", () => {
  ...
  it("교전중인 국가에 종전 제의 시 성공해야 함", () => {
    ...
    expect(delta.messages?.[0]?.meta?.action).toBe("propose_peace");
    // 로그 확인
    expect(delta.logs?.general?.[1][0]).toContain("종전 제의 서신");
  });

  it("선포중인 국가에 종전 제의 시 성공해야 함", () => {
  ...
  it("평화 상태에서는 종전 제의할 수 없어야 함", () => {
    const snapshot: WorldSnapshot = {
      ...baseSnapshot,
      diplomacy: {}, // 평화 상태 (외교 관계 없음)
    };
