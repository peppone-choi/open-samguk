...
export class NationAcceptNonAggressionCommand extends GeneralCommand {
  ...
  run(rng: RandUtil, snapshot: WorldSnapshot, actorId: number, args: Record<string, any>): WorldDelta {
    ...
    if (!iNation) return { logs: { general: { [actorId]: ['불가침 수락 실패: 소속 국가 정보를 찾을 수 없습니다.'] } } };

    const { destNationId, destGeneralId, year, month } = args;
    if (destNationId === undefined || destGeneralId === undefined || year === undefined || month === undefined) {
      return { logs: { general: { [actorId]: ['불가침 수락 실패: 필요한 정보가 누락되었습니다.'] } } };
    }

    const iDestNation = snapshot.nations[destNationId];
    if (!iDestNation) {
      return { logs: { general: { [actorId]: ['불가침 수락 실패: 대상 국가를 찾을 수 없습니다.'] } } };
    }

    const iDestGeneral = snapshot.generals[destGeneralId];
    if (!iDestGeneral || iDestGeneral.nationId !== destNationId) {
      return { logs: { general: { [actorId]: ['불가침 수락 실패: 제의 장수가 국가 소속이 아닙니다.'] } } };
    }
    ...
    if (reqMonth <= currentMonth) {
      return { logs: { general: { [actorId]: ['불가침 수락 실패: 이미 기한이 지났습니다.'] } } };
    }
    ...
    if (iDiplomacy) {
      if (iDiplomacy.state === '0') {
        return { logs: { general: { [actorId]: ['불가침 수락 실패: 아국과 이미 교전중입니다.'] } } };
      }
      if (iDiplomacy.state === '1') {
        return { logs: { general: { [actorId]: ['불가침 수락 실패: 아국과 이미 선포중입니다.'] } } };
      }
    }
    ...
    return {
      ...
      logs: {
        general: {
          [actorId]: [`【${iDestNation.name}】${josaWa} ${year}년 ${month}월까지 불가침에 성공했습니다.`],
          [destGeneralId]: [`【${iNation.name}】${JosaUtil.pick(iNation.name, '와')} ${year}년 ${month}월까지 불가침에 성공했습니다.`],
        },
      },
    };
  }
