...
export class GeneralDestructCommand extends GeneralSabotageCommand {
  ...
  override affectDestCity(rng: RandUtil, destCity: ICity): { cityDelta: Delta<ICity>, successMsg: string } {
    const defDamage = Math.min(rng.nextRangeInt(GameConst.sabotageDamageMin, GameConst.sabotageDamageMax), destCity.def);
    const wallDamage = Math.min(rng.nextRangeInt(GameConst.sabotageDamageMin, GameConst.sabotageDamageMax), destCity.wall);
