...
export class CityHealTrigger implements Trigger {
  ...
  execute(ctx: TriggerContext): TriggerResult {
    ...
    if (actor.injury > 0) {
      ...
      ctx.env["pre.치료"] = true;
      logs.push(`<C>${this.sourceName}</>을 펼쳐 스스로 치료합니다!`);
    }
  ...
  private findPatientsInCity(
    ctx: TriggerContext,
    actor: { id: number; cityId: number; nationId: number }
    ...
    for (const [id, general] of Object.entries(ctx.snapshot.generals)) {
      ...
      // 아군 한정 체크
      if (this.allyOnly && general.nationId !== actor.nationId) continue;
  ...
  private healPatients(
    ...
    for (const patient of patients) {
      if (!ctx.rand.nextBool(this.healProb)) continue;
  ...
  private formatHealLog(curedPatients: Array<{ name: string }>): string {
    ...
    if (curedPatients.length === 1) {
      ...
      return `<C>${this.sourceName}</>을 펼쳐 도시의 장수 <Y>${lastName}</>${josaUl} 치료합니다!`;
    }

    const otherCount = curedPatients.length - 1;
    return `<C>${this.sourceName}</>을 펼쳐 도시의 장수들 <Y>${lastName}</> 외 <C>${otherCount}</>명을 치료합니다!`;
  }
}
