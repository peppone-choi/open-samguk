...
describe("NewConstraints", () => {
    describe("AllowDiplomacyWithTermConstraint", () => {
        it("상태와 기한이 모두 만족하면 통과해야 함", () => {
            ...
            const diplomacy: Diplomacy = { id: 1, srcNationId: 1, destNationId: 2, state: "2", term: 12, meta: {} };
            expect(constraint.checkDiplomacy(diplomacy).kind).toBe("allow");
        });

        it("상태가 다르면 실패해야 함", () => {
            const constraint = new AllowDiplomacyWithTermConstraint("2", 12, "조건 미달");
            const diplomacy: Diplomacy = { id: 1, srcNationId: 1, destNationId: 2, state: "1", term: 12, meta: {} };
            expect(constraint.checkDiplomacy(diplomacy).kind).toBe("deny");
        });

        it("기한이 부족하면 실패해야 함", () => {
            const constraint = new AllowDiplomacyWithTermConstraint("2", 12, "조건 미달");
            const diplomacy: Diplomacy = { id: 1, srcNationId: 1, destNationId: 2, state: "2", term: 11, meta: {} };
            expect(constraint.checkDiplomacy(diplomacy).kind).toBe("deny");
        });
    });
    ...
    describe("AllowJoinDestNationConstraint", () => {
        const mockGeneral = (overrides: Partial<General> = {}): General => ({
            id: 1, name: "G1", nationId: 0, cityId: 1, npc: 0, gold: 0, rice: 0, leadership: 0, leadershipExp: 0, strength: 0, strengthExp: 0, intel: 0, intelExp: 0, politics: 0, politicsExp: 0, charm: 0, charmExp: 0, injury: 0, experience: 0, dedication: 0, officerLevel: 0, officerCity: 0, recentWar: 0, crew: 0, crewType: 0, train: 0, atmos: 0, dex: {}, age: 0, bornYear: 0, deadYear: 0, special: "", specAge: 0, special2: "", specAge2: 0, weapon: "", book: "", horse: "", item: "", turnTime: new Date(), recentWarTime: null, makeLimit: 0, killTurn: 0, block: 0, defenceTrain: 0, tournamentState: 0, lastTurn: {}, meta: {}, penalty: {}, ownerId: 1, troopId: 0, ...overrides
        });

        const mockNation = (overrides: Partial<Nation> = {}): Nation => ({
            id: 2, name: "N2", color: "", chiefGeneralId: 0, capitalCityId: 0, gold: 0, rice: 0, rate: 0, rateTmp: 0, tech: 0, power: 0, level: 1, gennum: 5, typeCode: "", scoutLevel: 0, strategicCmdLimit: 0, surrenderLimit: 0, spy: {}, aux: {}, meta: {}, warState: 0, ...overrides
        });
        ...
        it("정상 조건에서 통과해야 함", () => {
            ...
            const view = new SnapshotStateView({
                ...
                cities: {}, diplomacy: {}, troops: {}, messages: {}, gameTime: { year: 0, month: 0 }, env: {}
            });
