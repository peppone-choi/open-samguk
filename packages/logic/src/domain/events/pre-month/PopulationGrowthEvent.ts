  ...
  action(snapshot: WorldSnapshot): WorldDelta {
    ...
    for (const nation of Object.values(snapshot.nations)) {
      ...
      for (const city of nationCities) {
        ...
        // 내정 수치 계산
        const newAgri = Math.min(Math.floor(city.agri * (1 + genericRatio)), city.agriMax);
        const newComm = Math.min(Math.floor(city.comm * (1 + genericRatio)), city.commMax);
        const newSecu = Math.min(Math.floor(city.secu * (1 + genericRatio)), city.secuMax);
        const newDef = Math.min(Math.floor(city.def * (1 + genericRatio)), city.defMax);
        const newWall = Math.min(Math.floor(city.wall * (1 + genericRatio)), city.wallMax);
