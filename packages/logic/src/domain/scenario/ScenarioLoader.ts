...
export class ScenarioLoader {
    ...
    constructor(options: ScenarioLoaderOptions = {}) {
        ...
        this.scenarioPath = options.scenarioPath
            ?? join(projectRoot, 'legacy/hwe/scenario');
        this.defaultConfigPath = options.defaultConfigPath
            ?? join(this.scenarioPath, 'default.json');
     ...
     * 도시 엔티티 생성
     */
    private createCity(data: [number, string, string, number, number, number, number, number, number, string, number, number, string[]]): City {
    ...
    private createGeneral(
        ...
        const [
            affinity, name, _picturePath, nationName, locatedCity,
