 ...
 * 도시 규모 타입
 */
export const CityLevelSchema = z.enum(["특", "대", "중", "소", "진", "수", "관", "이"]);
export type CityLevel = z.infer<typeof CityLevelSchema>;

/**
 * 지역 타입
 */
export const RegionSchema = z.enum(["하북", "중원", "서북", "서촉", "남중", "초", "오월", "동이"]);
 ...
 * 게임 상수 오버라이드
 */
export const GameConstOverrideSchema = z
  .object({
...
export const BaseConditionSchema: z.ZodType<Condition> = z.lazy(() =>
  z.union([
    ...
    z.tuple([z.literal("DateRelative"), ConditionOperatorSchema, z.number()]),
    // 남은 국가 수 조건: ["RemainNation", op, count]
    z.tuple([z.literal("RemainNation"), ConditionOperatorSchema, z.number()]),
  ])
);
...
export const ActionSchema = z
  .array(z.unknown())
  .refine((arr): arr is [string, ...unknown[]] => arr.length >= 1 && typeof arr[0] === "string", {
 ...
 * [condition, ...actions]
 */
export const InitialEventSchema = z.array(z.unknown()).refine((arr) => arr.length >= 1, {
 ...
 */
export const GameEventDataSchema = z
  .array(z.unknown())
  .refine((arr) => arr.length >= 3 && typeof arr[0] === "string" && typeof arr[1] === "number", {
