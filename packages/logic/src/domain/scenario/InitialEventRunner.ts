...
export class InitialEventRunner {
    ...
    runInitialEvents(
        ...
        for (const event of scenario.initialEvents) {
            try {
                ...
                if (this.evaluateCondition(condition, snapshot, ctx)) {
    ...
    evaluateCondition(
        ...
        switch (type) {
            case 'and':
                return (args as Condition[]).every(c =>
                    this.evaluateCondition(c, snapshot, ctx)
                );

            case 'or':
                return (args as Condition[]).some(c =>
                    this.evaluateCondition(c, snapshot, ctx)
                );

            case 'Date':
                return this.evaluateDateCondition(args as [ConditionOperator, number | null, number | null], snapshot, ctx);

            case 'DateRelative':
                return this.evaluateDateRelativeCondition(args as [ConditionOperator, number], snapshot, ctx);
