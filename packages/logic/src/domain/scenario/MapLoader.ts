...
export class MapLoader {
    ...
    private parsePhpArray(content: string): CityInitData[] {
        ...
        // 각 행(도시) 파싱
        const rowRegex = /\[\s*(\d+)\s*,\s*'([^']+)'\s*,\s*'([^']+)'\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*'([^']+)'\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*\[([^\]]*)\]\s*\]/g;
    ...
    private getDefaultUnits(): UnitBuildData[] {
        return [
            ...
            },
            {
                ...
                descriptions: ['표준적인 보병입니다.', '보병은 방어특화이며,', '상대가 회피하기 어렵습니다.'],
            },
            {
                id: 1200,
                type: 2,  // ARCHER
