...
export interface Command {
  ...
  checkConstraints(rng: RandUtil, snapshot: WorldSnapshot, actorId: number, args: Record<string, any>, mode?: 'full' | 'precheck'): ConstraintResult;
...
export abstract class GeneralCommand implements Command {
   ...
   * 제약 조건을 검사한다.
   */
  public checkConstraints(rng: RandUtil, snapshot: WorldSnapshot, actorId: number, args: Record<string, any>, mode: 'full' | 'precheck' = 'full'): ConstraintResult {
  ...
  abstract run(rng: RandUtil, snapshot: WorldSnapshot, actorId: number, args: Record<string, any>): WorldDelta;
}
