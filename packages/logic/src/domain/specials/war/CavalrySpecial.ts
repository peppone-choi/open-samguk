...
export class CavalrySpecial extends BaseSpecial {
  ...
  static readonly type = [
    SpecialType.STAT_LEADERSHIP |
      ...
      SpecialType.ARMY_CAVALRY |
      SpecialType.STAT_NOT_INTEL,
    SpecialType.STAT_STRENGTH | SpecialType.REQ_DEXTERITY | SpecialType.ARMY_CAVALRY,
  ];
  ...
  onCalcDomestic(turnType: string, varType: string, value: number, aux?: DomesticAux): number {
    if (["징병", "모병"].includes(turnType)) {
      if (varType === "cost" && aux?.armType === T_CAVALRY) {
        return value * 0.9;
      }
    }
    return value;
