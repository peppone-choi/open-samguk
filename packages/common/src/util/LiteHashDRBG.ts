...
export class LiteHashDRBG implements RNG {
    ...
    protected genNextBlock(): void {
        ...
        // DataView.buffer는 ArrayBufferLike로 추론되므로, 해시 입력에 맞춰 ArrayBuffer로 단언한다.
    ...
    public nextBits(bits: number, baseBytes?: number): Uint8Array {
        ...
        // 음수/0 비트 요청은 허용하지 않는다.
        if (bytes <= 0) {
            throw new Error(`${bits} <= 0`);
        }

        // noUncheckedIndexedAccess 대응: 범위를 벗어나는 접근을 명시적으로 방지한다.
        const lastIndex = bytes - 1;
        const lastValue = result[lastIndex];
        if (lastValue === undefined) {
            throw new Error(`index ${lastIndex} is out of range`);
        }
        result[lastIndex] = lastValue & (0xff >> (8 - headBits));
        return result;
    }
